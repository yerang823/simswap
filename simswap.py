...
def main(pics, vids, output_folder, num_batch_size):
    ...
    for v in vids:
        ...
        for p in pics:
            ...
            sw_model = create_model(opt)
            sw_model.eval()

            detect_app = Face_detect_crop(name='antelope', root='./insightface_func/models')
            detect_app.prepare(ctx_id= 0, det_thresh=0.6, det_size=(640,640))
            ...
            with torch.no_grad():
                ...
                img_a_align_crop, _,_,_ = detect_app.get(img_a_whole,crop_size)
                ...
                latend_id = sw_model.netArc(img_id_downsample)
                #latend_id = model.module.netArc(img_id_downsample)
                latend_id = latend_id.detach().to('cpu')
                latend_id = latend_id/np.linalg.norm(latend_id,axis=1,keepdims=True)
                latend_id = latend_id.to('cuda')

                video_swap(opt.video_path, latend_id, sw_model, detect_app, opt.output_path, temp_results_dir=opt.temp_path, use_mask=opt.use_mask, n_batch_size=num_batch_size)
